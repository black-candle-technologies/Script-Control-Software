...
Portable serialization also removes acting identity, absolute project/watch/linked-FDX paths, linked-file timestamps, Git author preferences, and last-sync state. Snapshot sessions are sanitized for the same reason. No hosted service is required; collaboration uses portable projects plus optional Git/shared-drive sync.
