...
import { Menu, type MenuEntry } from "../ui.tsx";
...
export function DocumentTabs({ documents, state, onChange, onRequestRemove, onBeginExternalDrag, onEndExternalDrag, onInternalDrop, statusByDocumentId = {}, readOnly = false }: DocumentTabsProps) {
  ...
  const selectable = documents.filter((document) => Boolean(document.id));
  const focusTab = (index: number) => requestAnimationFrame(() => tablistRef.current?.querySelectorAll<HTMLElement>('[role="tab"]')[index]?.focus());
  const pickerItems: MenuEntry[] = selectable.map((document) => {
    const open = state.openDocumentIds.includes(document.id!);
    return {
      label: documentTitle(document),
      hint: state.activeDocumentId === document.id ? "Active" : open ? "Open" : "Closed",
      onSelect: () => onChange(open ? activateDocumentTab(state, document.id!) : openDocumentTab(state, document.id!)),
    };
  });
  return (
    <div className="document-tabs-shell">
        ...
        {state.openDocumentIds.map((documentId, index) => {
          ...
          return (
            <span key={documentId} className="document-tab-wrap">
              ...
              <button type="button" className="document-tab-menu-button" aria-label={`Actions for ${title}`} aria-haspopup="menu" aria-expanded={menuDocumentId === documentId} onClick={() => setMenuDocumentId(menuDocumentId === documentId ? undefined : documentId)}><Icon name="menu" size={13} /></button>
              {menuDocumentId === documentId ? (
                <div className="document-tab-menu" role="menu">
                  <button type="button" role="menuitem" className="menu-item" disabled={state.openDocumentIds.length <= 1} title={state.openDocumentIds.length <= 1 ? "Keep one screenplay open in this window" : undefined} onClick={() => { onChange(closeDocumentTab(state, documentId)); setMenuDocumentId(undefined); }}><span>Close view</span></button>
                  <button type="button" role="menuitem" className="menu-item danger" disabled={readOnly || !onRequestRemove} onClick={() => { onRequestRemove?.(documentId); setMenuDocumentId(undefined); }}><span>Remove from project…</span></button>
                </div>
              ) : null}
            </span>
          );
      ...
      {pickerItems.length > 1 ? (
        <Menu
          label={`All Screenplays (${pickerItems.length})`}
          icon={<Icon name="screenplay" size={12} />}
          items={pickerItems}
          align="start"
          buttonClassName="tool-btn document-picker"
        />
      ) : null}
    </div>
  );
