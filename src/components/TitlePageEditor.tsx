...
/** The four fields a printed title page centres; the rest sit in the lower detail block. */
const mastheadFields = new Set<TitlePageField>(["title", "credit", "author", "source"]);
...
export default function TitlePageEditor({ value, onChange, readOnly = false }: TitlePageEditorProps) {
  ...
  const renderField = (field: TitlePageField) => {
  ...
  return (
    <section className="title-page-editor" aria-labelledby="title-page-editor-heading">
      ...
      <div className="title-page-masthead">
        {TITLE_PAGE_FIELD_ORDER.filter((field) => mastheadFields.has(field)).map(renderField)}
      </div>
      ...
      <details className="title-page-imported-fields">
        ...
        <ol>
          {blocks.map((block, index) => (
            <li key={`${index}-${block.type}`}>
              <label><span>Field type</span><input value={block.type} readOnly={readOnly} placeholder="Untyped" onChange={(event) => setBlocks(replaceBlock(blocks, index, { ...block, type: event.target.value }))} /></label>
              <label><span>Text</span><textarea rows={Math.max(2, block.text.split("\n").length)} value={block.text} readOnly={readOnly} onChange={(event) => onChange(updateTitlePageBlockText(value, index, event.target.value))} /></label>
        ...
        {!readOnly ? <div className="title-page-block-actions"><button type="button" onClick={() => setBlocks([...blocks, { type: "Custom", text: "", metadata: {} }])}>Add custom paragraph</button></div> : null}
      </details>
    </section>
  );
}
